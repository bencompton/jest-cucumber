import { ParsedScenario, ParsedScenarioOutline, ScenarioFromStepDefinitions, ParsedStep } from '../models';
...
import { ErrorOptions, Options } from '../configuration';
...
export const ensureFeatureFileAndStepDefinitionScenarioHaveSameSteps = (
  ...
  if (options && options.errors && !(options.errors as ErrorOptions).stepsMustMatchFeatureFile) {
    return;
  }
  ...
  if (parsedStepCount !== stepDefinitionCount) {
    errors.push(
      `Scenario "${parsedScenario.title}" has ${parsedStepCount} step(s) in the feature file, but ${stepDefinitionCount} step definition(s) defined. Try adding the following code:\n\n${generateScenarioCode(parsedScenario)}`,
    );
  } else {
    parsedScenarioSteps.forEach((parsedStep, index) => {
      ...
      if (!stepFromStepDefinitions || !matchSteps(parsedStep.stepText, stepFromStepDefinitions.stepMatcher)) {
        errors.push(
          `Expected step #${index + 1} in scenario "${parsedScenario.title}" to match "${parsedStep.stepText}". Try adding the following code:\n\n${generateStepCode(parsedScenario.steps, index)}`,
        );
      }
    });
  }
