...
const findScenarioFromParsedFeature = (
  ...
  if (parsedScenarios) {
    matchingScenarios = parsedScenarios.filter(
      parsedScenario => parsedScenario.title.toLowerCase() === scenarioTitle.toLowerCase(),
    );
  }
...
const findScenarioFromStepDefinitions = (
  ...
  const matchingScenarios = scenariosFromStepDefinitions.filter(scenarioFromStepDefinitions => {
  ...
  if (matchingScenarios.length === 0 && errorOptions.scenariosMustMatchFeatureFile) {
    errors.push(
      `Feature file has a scenario titled "${scenarioTitle}", but no match found in step definitions. Try adding the following code:\n\n${generateScenarioCode(scenario)}`,
    );
